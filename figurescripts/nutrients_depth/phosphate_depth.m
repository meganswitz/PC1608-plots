% script plots phosphate with depth for all discrete bottling samples
%
...
xlim([0 2]);
ylim([0 370]);
xlabel(['\fontsize{24}Phosphate (' char(181) '\fontsize{24}M)']);
...
if castlines==1  
    for k=region
        ...
        if ismember(k,GBcasts)==1
            GBplot=plot(attribute(rows),p_b(rows),'LineStyle','-','Color',GB_l_color,'Linewidth',lw);
        elseif ismember(k,JBcasts)==1
            JBplot=plot(attribute(rows),p_b(rows),'LineStyle','-','Color',JB_l_color,'Linewidth',lw);
        elseif ismember(k,WBcasts)==1
            WBplot=plot(attribute(rows),p_b(rows),'LineStyle','-','Color',WB_l_color,'Linewidth',lw);
        elseif ismember(k,allothercasts)==1
            allotherplot=plot(attribute(rows),p_b(rows),'LineStyle','-','Color',allother_l_color,'Linewidth',lw);
        end
    end  
end
...
% ticks for x axis
xticks([0, .5, 1, 1.5, 2]);
xticklabels({'0.0', '0.5', '1.0', '1.5', '2.5'}); 
...
% finaldir=['/path/'];
% %savename
% savename='PC1608-totalphosphate-depth';
% %
